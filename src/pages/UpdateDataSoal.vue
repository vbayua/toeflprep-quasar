...
export default {
  ...
  mounted () {
    AdminService.getQuestion(this.$route.params.qid).then(
      question => {
        // console.log(question.data.question.question)
        this.question = question.data.question.question
        this.questionType = question.data.question.questionType
        this.paragraphs = question.data.question.paragraphs
        this.correctAnswer = question.data.question.correctAnswer
        this.option1 = question.data.question.answerOptions[0]
        this.option2 = question.data.question.answerOptions[1]
        this.option3 = question.data.question.answerOptions[2]
        this.option4 = question.data.question.answerOptions[3]
      },
      error => {
        console.log(error)
      }
    ...
    async onSubmit () {
      try {
        ...
        if (this.audiofile) {
          ...
          await AdminService.updateQuestion(this.$route.params.qid, data)
        } else {
          const data = {
            question: this.question,
            type: this.questionType,
